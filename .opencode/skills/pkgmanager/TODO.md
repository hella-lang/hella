...
- [ ] **P-1 Manifest + lockfile model** — `crates/hella-compiler/src/manifest.rs`: proper TOML parse of `[package]` + `[dependencies]` (`{ git, version, package? }`), short-name validation, `hella.lock` read/write (name → exact tag/SHA), keep backward compat with bare `name`/`version` manifests. Unit tests only, no network. (`toml` + `semver` deps.)
