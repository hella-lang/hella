...
| `stdlib_*.hll` (`string`, `num`, `math`, `env`, `fs`, `path`, `collections`, `fmt`, `io`, `rand`, `log`, `encoding`) | Standard-library tours: strings, parsing/conversion, math, environment, files, paths, vectors/maps, text composition, terminal IO, random generation, logging, hex/base64 (see `stdlib/README.md` for the module index) |
