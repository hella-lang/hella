...
## Async — structured concurrency (planned, not implemented)

Approved direction: explicit suspension, scoped child tasks, stackless state machines, and a single-threaded cooperative executor initially. `async` goes immediately before `do`: `int delayedValue(int value) async do ... end` and `void main() async do ... end`. Async calls must appear under `await` or `spawn` in v1; `spawn` starts concurrent work and returns a single-owner `task<T>` handle. No public lazy `Future<T>` or detached tasks in v1.

The compiler must automatically include the async runtime only when the program actually uses async code. Merely importing a module containing unused async declarations must not pull it in; synchronous programs must not acquire async scheduler initialization or async runtime dependencies. No user opt-in flag is required.
